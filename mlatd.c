...
static struct optionSpec optionSpecs[] = {
    {"canStop", OPTION_BOOLEAN},    {"log", OPTION_STRING},
    {"logFacility", OPTION_STRING}, {"mask", OPTION_BOOLEAN},
    {"maxAaSize", OPTION_INT},      {"maxDnaHits", OPTION_INT},
    {"maxGap", OPTION_INT},         {"maxNtSize", OPTION_INT},
    {"maxTransHits", OPTION_INT},   {"minMatch", OPTION_INT},
    {"repMatch", OPTION_INT},       {"seqLog", OPTION_BOOLEAN},
    {"ipLog", OPTION_BOOLEAN},      {"stepSize", OPTION_INT},
    {"tileSize", OPTION_INT},       {"trans", OPTION_BOOLEAN},
    {"syslog", OPTION_BOOLEAN},     {NULL, 0}};
...
/* Explain usage and exit. */
{
  errAbort(
      ...
      "   Where the files are in .2bit format\n"
      "To remove a server:\n"
      ...
      "To process one probe fa file against a .2bit format genome (not starting "
      "server):\n"
      ...
      "   -trans  Translate database to protein in 6 frames.  Note: it is "
      "best\n"
      ...
      "   -repMatch=N Number of occurrences of a tile (nmer) that trigger "
      "repeat masking the tile.\n"
      "               Default is %d.\n"
...
static void errorSafeQuery(boolean doTrans, boolean queryIsProt,
                           struct dnaSeq *seq, struct genoFind *gf,
                           struct genoFind *transGf[2][3], int connectionHandle,
                           char *buf)
  ...
  if (status == 0) /* Always true except after long jump. */
  {
    if (doTrans) {
      if (queryIsProt)
        ...
        transTransQuery(transGf, seq, connectionHandle, buf);
    } else
...
/* Load up index and hang out in RAM. */
{
  ...
  if (doTrans) {
    ...
    gfIndexTransNibsAndTwoBits(transGf, fileCount, seqFiles, minMatch, maxGap,
                               tileSize, repMatch, NULL, allowOneMismatch,
                               doMask, stepSize);
  } else {
    uglyf("starting untranslated server...\n");
    logInfo("setting up untranslated index");
    gf = gfIndexNibsAndTwoBits(fileCount, seqFiles, minMatch, maxGap, tileSize,
                               repMatch, NULL, allowOneMismatch, stepSize);
  }
          ...
          if (write(connectionHandle, buf, 1) == 1) {
            ...
            if (seq.size > 0) {
              ...
              if (gfReadMulti(connectionHandle, seq.dna, seq.size) !=
                ...
                errorSafeQuery(doTrans, queryIsProt, &seq, gf, transGf,
