...
int main(int argc, char* argv[])
{
	...
	for (size_t i = 0; i < queryCount; ++i) {
		...
		fprintf(stderr, "INFO: got res->capacity == %zu in %s\n", res->capacity, __func__);
		fprintf(stderr, "INFO: got res->tName == %p in %s\n", res->tName, __func__);
		fprintf(stderr, "INFO: got res->qName == %p in %s\n", res->qName, __func__);

		fprintf(stdout, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
			"matchCount", "mismatchCount", "repMatchCount", "nCount", "qInsertCount", "qInsertBaseCount",
			"tInsertCount", "tInsertBaseCount", "qStrand/tStrand");
		fprintf(stdout, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%c%c\n",
			hit->matchCount, hit->mismatchCount, hit->repMatchCount, hit->nCount,
			hit->qInsertCount, hit->qInsertBaseCount, hit->tInsertCount, hit->tInsertBaseCount,
			hit->qStrand, hit->tStrand);
