...
struct gfDb *newGfDb(char *dbFile, struct mlatParams *p)
{
	...
	bool bothSimpleProt =
		(p->tType == gftProt && p->qType == gftProt);
	// only valid for simple query and target types
	if (!(bothSimpleNuc || bothSimpleProt)) return NULL;
	...
	db->seqList = gfClientSeqList(dbCount, dbFiles, p->tType == gftProt, p->tType == gftDnaX,
	...
	// create indexer
	db->gf = gfIndexSeq(db->seqList, p->minMatch, p->maxGap, p->tileSize, p->repMatch, p->ooc,
			p->tType == gftProt, p->oneOff, FALSE, p->stepSize);
...
struct gfOutput *searchSeq(struct gfDb *db, char *querySeq, struct mlatParams *p)
{
  ...
  Bits *qMaskBits = maskQuerySeq(&seq0, p->tType == gftProt, p->qMask != NULL, lcMask);
  ...
  if (p->qType == gftRna || p->qType == gftRnaX)
	...
	struct gfOutput *gvo = gfOutputResult(p->minIdentity, p->qType == gftProt, p->tType == gftProt);
