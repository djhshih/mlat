 ...
 * generated repMask.h and repMask.sql.  This module links the database and the
 * RAM
 ...
 * be replaced at the next call to this function. */
{
  ...
  i = sqlSigned(row[12]);
  if (i < 0) {
    warn("WARNING: negative rEnd: %d %s:%d-%d %s", i, ret->qName, ret->qStart,
         ret->qEnd, ret->rName);
    ret->rEnd = 0;
  } else {
...
void repeatMaskOutOutput(struct repeatMaskOut *el, FILE *f, char sep,
                         char lastSep)
/* Print out repeatMaskOut.  Separate fields with sep. Follow last field with
   lastSep. */
{
