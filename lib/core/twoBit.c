  ...
  *pt = packDna4(last4);
  ...
  if (doMask) {
    ...
    if (twoBit->maskBlockCount > 0) {
      ...
      storeBlocksOfLower(dna, seq->size, twoBit->maskStarts, twoBit->maskSizes);
    }
  }
  return twoBit;
}
...
{
  return packedSize(twoBit->size) + sizeof(twoBit->size) +
 ...
 * stored only in index. */
{
  ...
  if (twoBit->nBlockCount > 0) {
    fwrite(twoBit->nStarts, sizeof(twoBit->nStarts[0]), twoBit->nBlockCount, f);
    fwrite(twoBit->nSizes, sizeof(twoBit->nSizes[0]), twoBit->nBlockCount, f);
  }
 ...
 * as will the hash. */
{
  ...
  if (tbf->seqCount != tbf->bpt->itemCount)
    errAbort("%s and %s don't have same number of sequences!", twoBitName,
             bptName);
  return tbf;
}
#endif
...
/* Get single sequence as two bit. */
{
  ...
  /* Read in blocks of N. */
  readBlockCoords(f, isSwapped, &(twoBit->nBlockCount), &(twoBit->nStarts),
                  &(twoBit->nSizes));
 ...
 * if doMask is true. */
{
  ...
  if (fragEnd == 0)
    fragEnd = seqSize;
  if (fragEnd > seqSize)
    errAbort("twoBitReadSeqFrag in %s end (%d) >= seqSize (%d)", name, fragEnd,
             seqSize);
  outSize = fragEnd - fragStart;
  if (outSize < 1)
    errAbort("twoBitReadSeqFrag in %s start (%d) >= end (%d)", name, fragStart,
             fragEnd);
  ...
  if (doMask) {
    ...
    if (maskBlockCount > 0) {
      int startIx =
          findGreatestLowerBound(maskBlockCount, maskStarts, fragStart);
 ...
 *     seqName:start-end */
{
  ...
  if (tbs->seqs != NULL) {
    ...
    for (tbss = tbs->seqs; tbss != NULL; tbss = tbss->next)
      slSafeAddHead(&list,
                    twoBitReadSeqFrag(tbf, tbss->name, tbss->start, tbss->end));
  } else {
    struct twoBitIndex *index;
    for (index = tbf->indexList; index != NULL; index = index->next)
...
boolean twoBitParseRange(char *rangeSpec, char **retFile, char **retSeq,
