...
struct dnaSeq *
gfiExpandAndLoadCached(struct gfRange *range, struct hash *tFileCache,
                       char *tSeqDir, int querySize, int *retTotalSeqSize,
                       boolean respectFrame, boolean isRc, int expansion)
 ...
 * know target size before loading.) */
{
  ...
  if (nibIsFile(fileName)) {
    ...
    target = nibLdPart(fileName, nib->f, nib->size, range->tStart,
