...
static void validateOption(char *name, char *val,
                           struct optionSpec *optionSpecs)
/* validate an option against a list of values */
{
  ...
  case OPTION_INT:
    ...
    if ((*val == '\0') || (*valEnd != '\0'))
      errAbort("value of -%s is not a valid integer: \"%s\"", name, val);
    break;
  case OPTION_LONG_LONG:
    ...
    if ((*val == '\0') || (*valEnd != '\0'))
      errAbort("value of -%s is not a valid long long: \"%s\"", name, val);
    break;
  case OPTION_FLOAT:
    if (val == NULL)
    ...
    if ((*val == '\0') || (*valEnd != '\0'))
      errAbort("value of -%s is not a valid float: \"%s\"", name, val);
    break;
  case OPTION_DOUBLE:
    if (val == NULL)
    ...
    discardMe = (long long)strtod(val, &valEnd);
    if ((*val == '\0') || (*valEnd != '\0'))
      errAbort("value of -%s is not a valid double: \"%s\"", name, val);
    break;
  default:
...
static void parseMultiOption(struct hash *hash, char *name, char *val,
                             struct optionSpec *spec)
/* process multiple instances of an option, requres that the optionSpec of the
   option */
{
  ...
  case OPTION_STRING:
    ...
    break;
  default:
    errAbort("UNIMPLEMENTED: multiple instances of a non-string option is not "
             "currently implemented");
  }
}

static boolean parseAnOption(struct hash *hash, char *arg,
                             struct optionSpec *optionSpecs)
 ...
 */
{
  ...
  /* We treat this=that as an option only if the '=' happens before any
   * non-alphanumeric
   * characters.  This lets us have URLs and SQL statements in the command line
   * even though
  ...
  if (optionSpecs == NULL)
    ...
    if (spec != NULL &&
        (spec->flags & OPTION_MULTI)) /* process multiple instances of option */
...
/* Return named option if in options hash, otherwise default. */
{
  ...
  if (optionSpecification != NULL) {
    ...
    if (spec != NULL && (spec->flags & OPTION_MULTI))
      errAbort("ERROR: optionVal cannot be used to get the value of an "
               "OPTION_MULTI");
  }
 ...
 * if not set. */
{
  ...
  if (lval > INT_MAX)
    errAbort("value of -%s is is too large: %ld, integer maximum is %d", name,
             lval, INT_MAX);
  if (lval < INT_MIN)
    errAbort("value of -%s is is too small: %ld, integer minimum is %d", name,
             lval, INT_MIN);
  return lval;
}
...
/* Return named option if in options hash, otherwise default. */
{
  ...
  if (optionSpecification == NULL)
    errAbort(
        "ERROR: optionMultiVal can only be used after optionInit is called "
