 ...
 * generated as_psl.h and as_psl.sql.  This module links the database and the
 * RAM
...
static char *createString =
    ...
    "that aren't repeats\n"
    "misMatches int unsigned not null,	# Number of bases that don't "
    "match\n"
    "repMatches int unsigned not null,	# Number of bases that match "
    "but are part of repeats\n"
    "nCount int unsigned not null,	# Number of 'N' bases\n"
    "qNumInsert int unsigned not null,	# Number of inserts in query\n"
    "qBaseInsert int unsigned not null,	# Number of bases inserted in "
    "query\n"
    "tNumInsert int unsigned not null,	# Number of inserts in target\n"
    "tBaseInsert int unsigned not null,	# Number of bases inserted in "
    "target\n"
    "strand char(2) not null,	# + or - for strand.  First character "
    "is query, second is target.\n"
    ...
    "blockCount int unsigned not null,	# Number of blocks in "
    "alignment\n"
...
/* Print out selected psl values.  Separate fields with sep. Follow last field
   with lastSep. */
...
/* declared here are printed if replacing an existing psl print function*/
{
  ...
  char *uformat =
      "<B>%s:</B> %u%c"; /* string for formatted print for unsigned variable */
  char *targName;
...
/* Write column info. */
{
  fputs("\n"
        ...
        " \tQ   \tQ    \tQ  \tT        \tT   \tT    \tT  \tblock\tblockSizes "
        "\tqStarts\t tStarts\n"
  ...
  if (!lineFileNext(lf, &line, &lineSize))
    ...
    if (startsWith("psLayout version", line)) {
      ...
      if (sameString(version, "3")) {
        ...
        errAbort("%s is version %s of psLayout, this program can only handle "
                 "through version 4",
                 fileName, version);
      }
      for (i = 0; i < 4; ++i) {
...
static void pslxFileOpenWithMetaConfig(char *fileName, bool isMetaUnique,
                                       enum gfType *retQueryType,
                                       enum gfType *retTargetType,
                                       struct lineFile **retLf, FILE *f)
/* Read header part of psl and make sure it's right.  Return
...
void pslxFileOpenWithMeta(char *fileName, enum gfType *retQueryType,
                          enum gfType *retTargetType, struct lineFile **retLf,
                          FILE *f)
/* Read header part of psl and make sure it's right.  Return
 * sequence types and file handle and send meta data to output file f */
{
  pslxFileOpenWithMetaConfig(fileName, FALSE, retQueryType, retTargetType,
                             retLf, f);
}

void pslxFileOpenWithUniqueMeta(char *fileName, enum gfType *retQueryType,
                                enum gfType *retTargetType,
                                struct lineFile **retLf, FILE *f)
/* Read header part of psl and make sure it's right.  Return
 * sequence types and file handle and send only unique meta data to output f */
{
  pslxFileOpenWithMetaConfig(fileName, TRUE, retQueryType, retTargetType, retLf,
                             f);
}
...
/* is psl a protein psl (are it's blockSizes and scores in protein space) */
{
  ...
  return (((psl->strand[1] == '+') &&
           (psl->tEnd ==
            psl->tStarts[lastBlock] + 3 * psl->blockSizes[lastBlock])) ||
          ((psl->strand[1] == '-') &&
...
struct psl *pslFromFakeFfAli(struct ffAli *ff, DNA *needle, DNA *haystack,
                             char strand, char *qName, int qSize, char *tName,
                             int tSize)
...
struct ffAli *pslToFfAli(struct psl *psl, struct dnaSeq *query,
                         struct dnaSeq *target, int targetOffset)
...
int pslWeightedIntronOrientation(struct psl *psl, struct dnaSeq *genoSeq,
                                 int offset)
 ...
 * Sequence should NOT be reverse complemented.  */
{
  ...
  if (psl->strand[1] == '-')
    errAbort("pslWeightedIntronOrientation doesn't support a negative target "
             "strand");
   ...
   */
{
  ...
  unsigned *tStarts = psl->tStarts, *qStarts = psl->qStarts,
...
/* Dump most of PSL to file - for debugging. */
{
  ...
  fprintf(f, "psl %s:%d-%d %s %s:%d-%d %d\n", psl->qName, psl->qStart,
          psl->qEnd, psl->strand, psl->tName, psl->tStart, psl->tEnd,
          psl->blockCount);
  for (i = 0; i < psl->blockCount; ++i)
...
/* print description of a PSL on first error */
{
  fprintf(out, "Error: invalid PSL: %s:%u-%u %s:%u-%u %s %s\n", psl->qName,
          psl->qStart, psl->qEnd, psl->tName, psl->tStart, psl->tEnd,
          psl->strand, pslDesc);
}

static void chkError(char *pslDesc, FILE *out, struct psl *psl, int *errCount,
                     char *format, ...)
...
static void chkBlkRanges(char *pslDesc, FILE *out, struct psl *psl, char *pName,
                         char *pLabel, char pCLabel, char pStrand,
...
/* check the target or query ranges in a PSL incrementing errorCnt */
{
  ...
  if ((pSize > 0) && (blkEnd > pSize))
    chkError(pslDesc, out, psl, errCount,
             ...
             blkEnd, pCLabel, pSize);
  if (gBlkStart < pStart)
    chkError(pslDesc, out, psl, errCount,
             "\t%s %s block %u start %u < %cStart %u\n", pName, pLabel, iBlk,
             gBlkStart, pCLabel, pStart);
  if (gBlkStart >= pEnd)
    chkError(pslDesc, out, psl, errCount,
             "\t%s %s block %u start %u >= %cEnd %u\n", pName, pLabel, iBlk,
             gBlkStart, pCLabel, pEnd);
  if (gBlkEnd < pStart)
    chkError(pslDesc, out, psl, errCount,
             "\t%s %s block %u end %u < %cStart %u\n", pName, pLabel, iBlk,
             gBlkEnd, pCLabel, pStart);
  if (gBlkEnd > pEnd)
    chkError(pslDesc, out, psl, errCount,
             "\t%s %s block %u end %u > %cEnd %u\n", pName, pLabel, iBlk,
             gBlkEnd, pCLabel, pEnd);
  if (iBlk > 0) {
    unsigned prevBlkEnd = pBlockStarts[iBlk - 1] + blockSizes[iBlk - 1];
    if (blkStart < prevBlkEnd)
      chkError(pslDesc, out, psl, errCount,
               "\t%s %s block %u start %u < previous block end %u\n", pName,
               pLabel, iBlk, blkStart, prevBlkEnd);
  }
}

static void chkRanges(char *pslDesc, FILE *out, struct psl *psl, char *pName,
                      char *pLabel, char pCLabel, char pStrand, unsigned pSize,
                      unsigned pStart, unsigned pEnd, unsigned blockCount,
                      unsigned *blockSizes, unsigned *pBlockStarts,
                      int blockSizeMult, int *errCount)
/* check the target or query ranges in a PSL, increment errorCnt */
{
  unsigned iBlk;
  if (pStart >= pEnd)
    chkError(pslDesc, out, psl, errCount, "\t%s %cStart %u >= %cEnd %u\n",
             pName, pCLabel, pStart, pCLabel, pEnd);
  if (pEnd > pSize)
    chkError(pslDesc, out, psl, errCount, "\t%s %cEnd %u >= %cSize %u\n", pName,
             pCLabel, pEnd, pCLabel, pSize);
  ...
  unsigned lastBlkEnd = pBlockStarts[blockCount - 1] +
                        (blockSizeMult * blockSizes[blockCount - 1]);
  if ((pStartStrand != pBlockStarts[0]) || (pEndStrand != lastBlkEnd))
    chkError(pslDesc, out, psl, errCount, "\t%s strand \"%c\" adjusted "
                                          "%cStart-%cEnd range %u-%u != block "
                                          "range %u-%u\n",
             pName, pStrand, pCLabel, pCLabel, pStartStrand, pEndStrand,
             pBlockStarts[0], lastBlkEnd);

  for (iBlk = 0; iBlk < blockCount; iBlk++)
    chkBlkRanges(pslDesc, out, psl, pName, pLabel, pCLabel, pStrand, pSize,
                 pStart, pEnd, iBlk, blockSizes, pBlockStarts, errCount);
}
...
{
  static char *VALID_STRANDS[] = {"+", "-", "++", "+-", "-+", "--", NULL};
  int i, errCount = 0;
  ...
  if (VALID_STRANDS[i] == NULL)
    chkError(pslDesc, out, psl, &errCount, "\tinvalid PSL strand: \"%s\"\n",
             psl->strand);

  /* check target */
  chkRanges(pslDesc, out, psl, psl->tName, "target", 't', pslTStrand(psl),
            psl->tSize, psl->tStart, psl->tEnd, psl->blockCount,
            psl->blockSizes, psl->tStarts, tBlockSizeMult, &errCount);

  /* check query */
  chkRanges(pslDesc, out, psl, psl->qName, "query", 'q', pslQStrand(psl),
            psl->qSize, psl->qStart, psl->qEnd, psl->blockCount,
            psl->blockSizes, psl->qStarts, 1, &errCount);

  return errCount;
}

struct hash *readPslToBinKeeper(char *sizeFileName, char *pslFileName)
/* read a list of psls and return results in hash of binKeeper structure for
   fast query*/
{
  struct binKeeper *bk;
  struct psl *psl;
...
/* remove leading or trailing indels from alignment */
{
  ...
  while ((aliSize > 0) &&
         (isDelChar(qStr[aliSize - 1]) || isDelChar(tStr[aliSize - 1]))) {
...
static void accumCounts(struct psl *psl, char prevQ, char prevT, char q, char t,
                        unsigned options)
...
struct psl *pslFromAlign(char *qName, int qSize, int qStart, int qEnd,
                         char *qString, char *tName, int tSize, int tStart,
                         int tEnd, char *tString, char *strand,
                         unsigned options)
 ...
 * triming leading and trailing indels.*/
{
  ...
  psl = pslNew(qName, qSize, qStart, qEnd, tName, tSize, tStart, tEnd, strand,
               blockSpace, 0);
...
/* create a PSL from a GFF3-style cigar formatted alignment */
{
  ...
  int blocksAlloced = 4;
  struct psl *psl = pslNew(qName, qSize, qStart, qEnd, tName, tSize, tStart,
                           tEnd, strand, blocksAlloced, 0);
