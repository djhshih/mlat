...
/* Add name of given size to hash (no need to be zero terminated) */
{
  ...
  hash->elCount += 1;
  if (hash->autoExpand &&
      hash->elCount > (int)(hash->size * hash->expansionFactor)) {
    /* double the size */
...
/* Increment integer value in hash */
{
  ...
  if (hel == NULL) {
    ...
    /* The much simpler ++hel->val works for gnu C, but really adding one to a
     * void pointer
...
/* Returns new hash table. Uses local memory optionally. */
{
  ...
  hash->expansionFactor =
      defaultExpansionFactor; /* Expand when elCount > size*expansionFactor */
  return hash;
}
 ...
 * hash table while this is being used. */
{
  ...
  if (cookie->nextEl == NULL) {
    for (cookie->idx++; (cookie->idx < cookie->hash->size) &&
                        (cookie->hash->table[cookie->idx] == NULL);
         cookie->idx++)
      continue; /* empty body */
...
/* print statistic about a hash table */
{
  ...
  fprintf(fh, "occupied\t%d\t%0.4f\n", occupiedCnt,
          ((hash->size == 0) ? 0.0 : ((float)occupiedCnt) / hash->size));
