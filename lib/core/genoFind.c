...
static struct genoFind *gfNewEmpty(int minMatch, int maxGap, int tileSize,
                                   int stepSize, int maxPat, char *oocFile,
 ...
 */
{
  struct genoFind *gf = gfNewEmpty(minMatch, maxGap, tileSize, stepSize, maxPat,
                                   oocFile, FALSE, allowOneMismatch);
  int i;
  ...
  if (allowOneMismatch)
    errAbort(
        "Don't currently support allowOneMismatch in gfIndexNibsAndTwoBits");
  if (stepSize == 0)
...
static void
transIndexBothStrands(struct dnaSeq *seq, struct genoFind *transGf[2][3],
                      bits32 offset[2][3], int sourceIx, char *fileName)
...
void gfIndexTransNibsAndTwoBits(struct genoFind *transGf[2][3], int fileCount,
                                char *fileNames[], int minMatch, int maxGap,
                                int tileSize, int maxPat, char *oocFile,
...
/* Make translated (6 frame) index for all .nib and .2bit files. */
{
  ...
  if (allowOneMismatch)
    errAbort("Don't currently support allowOneMismatch in "
             "gfIndexTransNibsAndTwoBits");
  /* Allocate indices for all reading frames. */
  for (isRc = 0; isRc <= 1; ++isRc) {
    for (frame = 0; frame < 3; ++frame) {
      transGf[isRc][frame] = gf =
          gfNewEmpty(minMatch, maxGap, tileSize, stepSize, maxPat, oocFile,
                     TRUE, allowOneMismatch);
    }
  }
...
/* Check that each sequence has a unique name. */
{
  ...
  for (seq = seqList; seq != NULL; seq = seq->next) {
    if (hashLookup(hash, seq->name))
      errAbort("Error: sequence name %s is repeated in the database, all names "
               "must be unique.",
               seq->name);
...
struct genoFind *gfIndexSeq(bioSeq *seqList, int minMatch, int maxGap,
                            int tileSize, int maxPat, char *oocFile,
                            boolean isPep, boolean allowOneMismatch,
                            boolean maskUpper, int stepSize)
 ...
 * be unindexed. */
{
  ...
  if (stepSize == 0)
    stepSize = tileSize;
  if (gf->segSize > 0) {
    gfLargeIndexSeq(gf, seqList, minMatch, maxGap, tileSize, maxPat, oocFile,
                    isPep, maskUpper);
  } else {
    gfSmallIndexSeq(gf, seqList, minMatch, maxGap, tileSize, maxPat, oocFile,
                    isPep, maskUpper);
  }
  return gf;
}
...
static void targetClump(struct genoFind *gf, struct gfClump **pClumpList,
                        struct gfClump *clump)
...
static struct gfClump *clumpNear(struct genoFind *gf, struct gfClump *oldClumps,
                                 int minMatch)
...
/* Clump together hits according to parameters in gf. */
{
  ...
  for (clump = clumpList; clump != NULL; clump = clump->next) /* uglyf */
  {
    uglyf(" %d %d %s %d %d (%d hits)\n", clump->qStart, clump->qEnd,
          clump->target->seq->name, clump->tStart, clump->tEnd,
          clump->hitCount);
  }
#endif /* DEBUG */
  freez(&buckets);
...
static struct gfHit *
gfFastFindDnaHits(struct genoFind *gf, struct dnaSeq *seq, Bits *qMaskBits,
                  int qMaskOffset, struct lm *lm, int *retHitCount,
 ...
 * case for DNA that is in an unsegmented index. */
{
  ...
  for (i = tileSizeMinusOne; i < size; ++i) {
    ...
    if (listSize != 0) {
      ...
      if (qMaskBits == NULL ||
          bitCountRange(qMaskBits, qStart + qMaskOffset, gf->tileSize) == 0) {
        tList = gf->lists[bits];
        for (j = 0; j < listSize; ++j) {
          int tStart = tList[j];
          if (target == NULL || (target == findSource(gf, tStart) &&
...
static struct gfHit *
gfStraightFindHits(struct genoFind *gf, aaSeq *seq, Bits *qMaskBits,
                   int qMaskOffset, struct lm *lm, int *retHitCount,
                   struct gfSeqSource *target, int tMin, int tMax)
 ...
 * index where hits match exactly. */
{
  ...
  for (i = 0; i <= lastStart; ++i) {
    ...
    if (listSize > 0) {
      qStart = i;
      if (qMaskBits == NULL ||
          bitCountRange(qMaskBits, qStart + qMaskOffset, tileSize) == 0) {
        tList = gf->lists[tile];
        for (j = 0; j < listSize; ++j) {
          int tStart = tList[j];
          if (target == NULL || (target == findSource(gf, tStart) &&
...
static struct gfHit *
gfStraightFindNearHits(struct genoFind *gf, aaSeq *seq, Bits *qMaskBits,
                       int qMaskOffset, struct lm *lm, int *retHitCount,
                       struct gfSeqSource *target, int tMin, int tMax)
/* Find hits associated with one sequence in a non-segmented
        ...
        for (varVal = 0; varVal < alphabetSize; ++varVal) {
          if (varVal != avoid) {
            ...
            if (listSize > 0) {
              qStart = i;
              if (qMaskBits == NULL ||
                  bitCountRange(qMaskBits, qStart + qMaskOffset, tileSize) ==
                      0) {
                tList = gf->lists[tile];
                for (j = 0; j < listSize; ++j) {
                  int tStart = tList[j];
...
static struct gfHit *
gfSegmentedFindHits(struct genoFind *gf, aaSeq *seq, Bits *qMaskBits,
                    int qMaskOffset, struct lm *lm, int *retHitCount,
                    struct gfSeqSource *target, int tMin, int tMax)
 ...
 * index. */
{
  ...
  for (i = 0; i <= lastStart; ++i) {
    if (qMaskBits == NULL ||
        bitCountRange(qMaskBits, i + qMaskOffset, tileSize) == 0) {
...
static struct gfHit *
gfSegmentedFindNearHits(struct genoFind *gf, aaSeq *seq, Bits *qMaskBits,
                        int qMaskOffset, struct lm *lm, int *retHitCount,
                        struct gfSeqSource *target, int tMin, int tMax)
...
static struct gfHit *
gfFindHitsWithQmask(struct genoFind *gf, bioSeq *seq, Bits *qMaskBits,
                    int qMaskOffset, struct lm *lm, int *retHitCount,
                    struct gfSeqSource *target, int tMin, int tMax)
/* Find hits associated with one sequence soft-masking seq according to
 * qMaskBits.
 * The hits will be in genome rather than chromosome coordinates. */
{
  ...
  if (gf->segSize == 0 && !gf->isPep && !gf->allowOneMismatch) {
    hitList = gfFastFindDnaHits(gf, seq, qMaskBits, qMaskOffset, lm,
    ...
    if (gf->segSize == 0) {
      if (gf->allowOneMismatch) {
        ...
        hitList = gfStraightFindHits(gf, seq, qMaskBits, qMaskOffset, lm,
                                     retHitCount, target, tMin, tMax);
      }
    } else {
      if (gf->allowOneMismatch) {
        hitList = gfSegmentedFindNearHits(gf, seq, qMaskBits, qMaskOffset, lm,
                                          retHitCount, target, tMin, tMax);
      } else {
        hitList = gfSegmentedFindHits(gf, seq, qMaskBits, qMaskOffset, lm,
                                      retHitCount, target, tMin, tMax);
      }
    }
  }
...
/* Find clumps associated with one sequence soft-masking seq according to
   qMaskBits */
{
...
struct gfHit *gfFindHitsInRegion(struct genoFind *gf, bioSeq *seq,
                                 Bits *qMaskBits, int qMaskOffset,
                                 struct lm *lm, struct gfSeqSource *target,
                                 int tMin, int tMax)
 ...
 * coordinates as hits inside of clumps usually are.  */
{
  ...
  hitList = gfFindHitsWithQmask(gf, seq, qMaskBits, qMaskOffset, lm, &hitCount,
                                target, tMin + targetStart, tMax + targetStart);
  for (hit = hitList; hit != NULL; hit = hit->next)
    hit->tStart -= targetStart;
  return hitList;
}

struct gfClump *gfFindClumps(struct genoFind *gf, bioSeq *seq, struct lm *lm,
                             int *retHitCount)
...
void gfTransFindClumps(struct genoFind *gfs[3], aaSeq *seq,
                       struct gfClump *clumps[3], struct lm *lm,
                       int *retHitCount)
...
void gfTransTransFindClumps(struct genoFind *gfs[3], aaSeq *seqs[3],
                            struct gfClump *clumps[3][3], struct lm *lm,
                            int *retHitCount)
...
void gfMakeOoc(char *outName, char *files[], int fileCount, int tileSize,
               bits32 maxPat, enum gfType tType)
...
static void mergeAdd(struct binKeeper *bk, int start, int end,
...
static struct gfClump *
pcrClumps(struct genoFind *gf, char *fPrimer, int fPrimerSize, char *rPrimer,
          int rPrimerSize, int minDistance, int maxDistance)
...
struct gfClump *gfPcrClumps(struct genoFind *gf, char *fPrimer, int fPrimerSize,
                            char *rPrimer, int rPrimerSize, int minDistance,
                            int maxDistance)
