...
static struct memHandler defaultMemHandler =
    ...
    {
        NULL, defaultAlloc, defaultFree, defaultRealloc,
};
...
static size_t maxAlloc = (size_t)128 * 8 * 1024 * 1024 * (sizeof(size_t) / 4) *
                         (sizeof(size_t) / 4) *
                         (sizeof(size_t) / 4 * (sizeof(size_t) / 4));
 ...
 * not initialized to zero. */
{
  ...
  if ((pt = mhStack->alloc(size)) == NULL)
    errAbort(
        "needLargeMem: Out of memory - request size %llu bytes, errno: %d\n",
...
/* No checking on size.  Memory not initted. */
{
  ...
  if ((pt = mhStack->alloc(size)) == NULL)
    errAbort("needHugeMem: Out of huge memory - request size %llu bytes, "
             "errno: %d\n",
             (unsigned long long)size, errno);
  return pt;
}
...
static int carefulAlignSize; /* Alignment size for machine - 8 bytes for DEC
                                alpha, 4 for Sparc. */
static int carefulAlignAdd;  /* Do aliSize =
                                *(unaliSize+carefulAlignAdd)&carefulAlignMask);
                                */
...
/* Keep one of these for each outstanding memory block.   It's a doubly linked
   list. */
{
...
/* Check cookies and free. */
{
  ...
  if (memcmp(pEndCookie, cmbEndCookie, sizeof(cmbEndCookie)) != 0)
    errAbort("Bad end cookie %x%x%x%x freeing %llx\n", pEndCookie[0],
             pEndCookie[1], pEndCookie[2], pEndCookie[3], ptrToLL(vpt));
...
{
  int maxPieces =
      10000000; /* Assume no more than this many pieces allocated. */
  struct carefulMemBlock *cmb;
  char *pEndCookie;
  size_t size;
  ...
  for (cmb = (struct carefulMemBlock *)(cmbAllocedList->head);
       cmb->next != NULL; cmb = cmb->next) {
    size = cmb->size;
    pEndCookie = (((char *)(cmb + 1)) + size);
    if (cmb->startCookie != cmbStartCookie)
    ...
    if (memcmp(pEndCookie, cmbEndCookie, sizeof(cmbEndCookie)) != 0)
      errAbort("Bad end cookie %x%x%x%x checking %llx\n", pEndCookie[0],
               pEndCookie[1], pEndCookie[2], pEndCookie[3], ptrToLL(cmb + 1));
    if (--maxPieces == 0)
...
static struct memHandler carefulMemHandler =
    /* Default memory handler. */
    {
        NULL, carefulAlloc, carefulFree, carefulRealloc,
};
