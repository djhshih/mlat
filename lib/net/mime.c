...
/*
 * Note: MIME is a nested structure that makes a tree that streams in
 * depth-first.
 */

#define MAXPARTSIZE                                                            \
  64LL * 1024 * 1024 *                                                         \
      1024 /* max size before gets put in a tempfile to save memory. It        \
              currently has been set so large that it will not be triggered.   \
              */
#define MAXPARTLINESIZE                                                        \
  1024 /* header lines should be small, so bad if bigger than this */
#define MAXDATASIZE                                                            \
  64LL * 1024 * 1024 * 1024 /* max size allowable for large uploads */
...
/* debug dump */
{
  ...
  fprintf(stderr, "b->i  =%lu "
                  ...
                  "%d "
                  "\n",
          (unsigned long)b->i, (unsigned long)b->eop, (unsigned long)b->eod,
          (unsigned long)b->eoi, (unsigned long)b->eom, b->boundary, b->blen);
  fprintf(stderr, "*");
  for (i = 0; i < MIMEBUFSIZE; ++i) {
    fprintf(stderr, "%c",
            (b->buf[i] < 31 || (unsigned)b->buf[i] > 127) ? '.' : b->buf[i]);
  }
  fprintf(stderr, "\n\n");
}
#endif
...
static void getChunkMB(struct mimeBuf *b, char **address, int *size,
                       boolean *hasZeros)
/* Pass back address and size of chunk, and whether it contains embedded zeros.
   The chunk is the largest piece of data left in the buffer up to the eod or
   eop. */
{
...
static void readPartHeaderMB(struct mimeBuf *b, struct mimePart *p,
                             char *altHeader)
   ...
   saves the header settings in a hash.  */
{
  ...
  do {
    ...
    if (!val)
      errAbort(
          "readPartHeaderMB error - header-line colon not found, line=[%s]",
          line);
    *val = 0;
    val++;
    key = trimSpaces(key);
    // since the hash is case-sensitive, convert to lower case for ease of
    // matching
    tolowers(key);
    val = trimSpaces(val);
 ...
 * freeMem the returned string when done */
{
  ...
  while (TRUE) {
    ...
    if (i >= sizeof(value))
      errAbort(
          "error: main value too long (>%lu) in MIME header Content-type:%s",
          (unsigned long)sizeof(value), header);
  }
  value[i] = 0;
 ...
 * freeMem the returned string when done */
{
  ...
  while (TRUE) {
    ...
    if (i >= sizeof(value))
      errAbort(
          "error: %s= value too long (>%lu) in MIME header Content-type:%s",
          field, (unsigned long)sizeof(value), header);
  }
  value[i] = 0;
 ...
 */
{
  ...
  if (sameOk(altHeader,
             "autoBoundary")) { /* process things with no explicit header.
                                 *  look for *MIME* \n\n-- */
  ...
  if (ct && startsWith("multipart/", ct)) {
    ...
    if (strlen(boundary) >= MAXBOUNDARY)
      errAbort("error: boundary= value too long in MIME header Content-type:%s",
               ct);
    safef(bound, sizeof(bound), "--%s",
          boundary); /* do not prepend CRLF to boundary yet */
    freez(&boundary);
    ...
    while (TRUE) {
      ...
      if (c1 == '-') {
        ...
        if (c2 == '-')
          ...
          errAbort("expected -- after boundary %s, but found %c%c in MIME",
                   boundary, c1, c2);
      }
      if (nlType == nlt_dos)
        c2 = getcMB(b);
      ...
      case nlt_dos:
        if (c1 == 0x0d && c2 == 0x0a)
          ...
          errAbort("expected CRLF after boundary %s, but found %c%c in MIME",
                   boundary, c1, c2);
      case nlt_unix:
        if (c1 == 0x0a)
          break;
        else
          errAbort("expected LF after boundary %s, but found %c in MIME",
                   boundary, c1);
      case nlt_mac:
        if (c1 == 0x0d)
          break;
        else
          errAbort("expected CR after boundary %s, but found %c in MIME",
                   boundary, c1);
      default:
    ...
    // debug
    // ct = hashFindVal(p->hdr,"content-transfer-encoding");  /* use lowercase
    // key */
    ...
    while (TRUE) {
      ...
      if (p->size > MAXDATASIZE)
        errAbort("max data size allowable for upload in MIME exceeded %llu",
