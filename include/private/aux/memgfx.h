...
// BIGENDIAN machines:
    ...
    defined(sparc) || defined(__ppc__) || defined(__s390__) ||                 \
    defined(__s390x__)
...
#define MAKECOLOR_32(r, g, b)                                                  \
  (((unsigned int)0xff) | ((unsigned int)b << 8) | ((unsigned int)g << 16) |   \
   ((unsigned int)r << 24))
...
#define MAKECOLOR_32(r, g, b)                                                  \
  (((unsigned int)0xff << 24) | ((unsigned int)b << 16) |                      \
...
Color mgFindColor(struct memGfx *mg, unsigned char r, unsigned char g,
                  unsigned char b);
/* Returns closest color in color map to rgb values.  If it doesn't
 * already exist in color map and there's room, it will create
 * exact color in map. */

Color mgClosestColor(struct memGfx *mg, unsigned char r, unsigned char g,
                     unsigned char b);
/* Returns closest color in color map to r,g,b */

Color mgAddColor(struct memGfx *mg, unsigned char r, unsigned char g,
                 unsigned char b);
...
INLINE void mgPutDot(struct memGfx *mg, int x, int y, Color color) {
  if ((x) >= (mg)->clipMinX && (x) < (mg)->clipMaxX && (y) >= (mg)->clipMinY &&
      (y) < (mg)->clipMaxY) {
    switch (mg->writeMode) {
...
#define mgGetDot(mg, x, y)                                                     \
  ((x) >= (mg)->clipMinX && (x) < (mg)->clipMaxX && (y) >= (mg)->clipMinY &&   \
   (y) < (mg)->clipMaxY)                                                       \
      ? _mgGetDot(mg, x, y)                                                    \
      : 0
...
void mgDrawBox(struct memGfx *mg, int x, int y, int width, int height,
               Color color);
...
typedef void (*TextBlit)(int bitWidth, int bitHeight, int bitX, int bitY,
                         ...
                         struct memGfx *dest, int destX, int destY, Color color,
...
/* Collection of fonts from here and there.  The mgTinyFont() and mgSmallFont()
 * are uniq
...
void mgText(struct memGfx *mg, int x, int y, Color color, MgFont *font,
            char *text);
...
void mgFillUnder(struct memGfx *mg, int x1, int y1, int x2, int y2, int bottom,
                 Color color);
...
void mgCircle(struct memGfx *mg, int xCen, int yCen, int rad, Color color,
              boolean filled);
...
struct rgbColor mgRgbTransformHsl(struct rgbColor in, double h, double s,
                                  double l);
...
struct rgbColor mgRgbTransformHsv(struct rgbColor in, double h, double s,
                                  double v);
/* Transform rgb 'in' value using
 *   hue shift 'h' (0..360 degrees),
 *   saturation scale 's', and
