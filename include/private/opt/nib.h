...
#define NIB_MASK_MIXED                                                         \
  0x01 /* Read uses case to indicate masking.                                  \
        * Write sets mask bit for lower-case */
#define NIB_MASK_MAP                                                           \
  0x02 /* Read builds dnaSeq->mask bit map. Write                              \
        * uses mask to set mask bases.  Note: the                              \
        * bit map indicates which bases are not repeats                        \
        */
#define NIB_BASE_NAME                                                          \
  0x04 /* Return a sequence name that is the base name                         \
        * the file. */
...
struct nibInfo *nibInfoFromCache(struct hash *hash, char *nibDir,
                                 char *nibName);
