 ...
 *
 *    struct pipeline *pl = pipelineOpen(cmds, pipelineWrite|pipelineAppend,
 * outFilePath, stderrFd);
 */
#ifndef PIPELINE_H
#define PIPELINE_H
#include <stdio.h>
...
{ pipelineRead = 0x01,    /* read from pipeline */
  ...
  pipelineAppend =
      0x10, /* Append to output file (used only with pipelineWrite) */
 ...
 * is specified, the output of the pipeline is readable from the pipeline
 * object.  If pipelineWrite is specified, the input of the pipeline is
 * writable from the pipeline object.  If stderrFile is NULL, stderr is
 * inherited,
