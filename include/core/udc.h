...
struct udcFile;
/* Handle to a cached file.  Inside of structure mysterious unless you are
 * udc.c. */
...
char *udcFileReadAll(char *url, char *cacheDir, size_t maxSize,
                     size_t *retSize);
...
struct lineFile *udcWrapShortLineFile(char *url, char *cacheDir,
                                      size_t maxSize);
...
/* Prefix used by convention to indicate a file should be accessed via udc.
 * This is
 * followed by the local path name or a url, so in common practice you see
 * things like:
